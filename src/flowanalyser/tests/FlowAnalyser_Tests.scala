...
"""
    ...
    val r = FlowAnalyser.matchAgainstPat(pP("S(x)"), pT("S(A(S(x1)))"), SParsers.parseTreeGrammar(tgs))
...
g -> Z();
"""
    val r = FlowAnalyser.matchAgainstPat(pP("S(x)"), Var("x1"), SParsers.parseTreeGrammar(tgs))
    val r_str = "Map(x -> S(g))"
    //println(r.mkString(";"))
    assertEquals(r.mkString(";"), r_str)
  }
...
R1 -> S(S(x2));
"""
    val r = FlowAnalyser.matchAgainstPat(pP("S(x)"), RuleName("R1"), SParsers.parseTreeGrammar(tgs))
...
R1 -> Nil();
"""
    val r = FlowAnalyser.matchAgainstPat(pP("Cons(x,y)"), RuleName("R1"), SParsers.parseTreeGrammar(tgs))
...
"""
  ...
  def FlowAnalyser_Match_test9(): Unit = {
    var r = FlowAnalyser.matchAgainstPat(pP("S(x)"), pT("S(S(S(Z())))"), TreeGrammar.create())
    //println(r)
  ...
  def FlowAnalyser_Match_test10(): Unit = {
    var r = FlowAnalyser.matchAgainstPat(pP("S(x)"), pT("A(Z())"), TreeGrammar.create())
    //println(r)
...
"""
    ...
    val r = FlowAnalyser.matchAgainstPat(pP("S(x)"), RuleName("R6"), SParsers.parseTreeGrammar(tgs))
    //println(r)
...
"""
    ...
    val tg = SParsers.parseTreeGrammar(tgs)
    val f1 = FlowAnalyser.isReachable(pP("False()"), RuleName("R0"), tg)
    val f2 = FlowAnalyser.isReachable(pP("True()"), RuleName("R0"), tg)
    //println(f)
    assertFalse(f1)
    assertTrue(f2)
  }
